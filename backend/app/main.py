...
from app.routers.user import account  # 👤 Account Deletion & Export
...
app.include_router(account.router, prefix="", tags=["account"])  # Account deletion & export (router já tem /account prefix)
...
app.include_router(address_book.router, prefix="/address-book", tags=["address-book"])  # Address Book - Agenda de endereços
app.include_router(swap.router, prefix="", tags=["swap"])  # Swap/DEX - Troca de tokens via agregador
app.include_router(earnpool.router, prefix="", tags=["earnpool"])  # EarnPool - Pool de liquidez/rendimentos
app.include_router(earnpool_admin.router, prefix="", tags=["earnpool-admin"])  # EarnPool Admin
