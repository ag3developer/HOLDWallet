...
const QuickStat: React.FC<{
  ...
  <button
    onClick={onClick}
    className='group relative overflow-hidden flex items-center gap-2 sm:gap-3 p-2.5 sm:p-4 bg-white dark:bg-gray-800/80 rounded-xl sm:rounded-2xl hover:shadow-lg transition-all duration-300 w-full text-left border border-gray-100 dark:border-gray-700/50'
  >
    <div
    ...
    />
    <div
      className={`relative p-1.5 sm:p-2.5 rounded-lg sm:rounded-xl bg-gradient-to-br ${bgGradient}`}
    >
      <Icon className={`w-3.5 h-3.5 sm:w-5 sm:h-5 ${color}`} />
    </div>
    <div className='relative flex-1 min-w-0'>
      <p className='text-[10px] sm:text-xs text-gray-500 dark:text-gray-400 font-medium'>{label}</p>
      <p className={`text-base sm:text-xl font-bold ${color}`}>{value}</p>
      {subtitle && (
        <p className='text-[8px] sm:text-[10px] text-gray-400 dark:text-gray-500 truncate'>
          {subtitle}
        </p>
      )}
    </div>
    <ChevronRight className='relative w-3 h-3 sm:w-4 sm:h-4 text-gray-400 dark:text-gray-500 group-hover:translate-x-1 transition-transform' />
  </button>
)
...
const InsightBadge: React.FC<{
  ...
  return (
    <div
      className={`flex items-center gap-2 sm:gap-3 px-2.5 sm:px-4 py-2 sm:py-3 rounded-lg sm:rounded-xl border ${styles[type]} backdrop-blur-sm`}
    >
...
const AIInsightsCard: React.FC<AIInsightsCardProps> = ({
  ...
  if (loading) {
    return (
      <div className='relative overflow-hidden p-4 sm:p-8 bg-white dark:bg-gray-900/80 rounded-2xl sm:rounded-3xl border border-gray-200 dark:border-gray-700/50 shadow-xl'>
        <div className='absolute inset-0 bg-gradient-to-br from-blue-500/5 via-purple-500/5 to-indigo-500/5' />
        <div className='relative flex flex-col items-center justify-center py-6 sm:py-8 space-y-3 sm:space-y-4'>
          <div className='relative'>
            ...
            <div className='relative p-3 sm:p-4 bg-gradient-to-br from-blue-500/20 to-purple-500/20 rounded-full'>
  ...
  return (
    <div className='relative overflow-hidden p-4 sm:p-6 bg-white dark:bg-gray-900/80 rounded-2xl sm:rounded-3xl border border-gray-200 dark:border-gray-700/50 shadow-xl'>
      ...
      <div className='relative flex items-center justify-between mb-4 sm:mb-6'>
        <div className='flex items-center gap-2 sm:gap-4'>
          <div className='relative'>
            <div className='absolute inset-0 bg-gradient-to-br from-blue-500 to-purple-600 rounded-xl sm:rounded-2xl blur-lg opacity-40' />
            ...
            <div className='absolute -top-1 -right-1 p-0.5 sm:p-1 bg-yellow-400 rounded-full shadow-lg'>
              <Sparkles className='w-2 h-2 sm:w-3 sm:h-3 text-yellow-900' />
            </div>
          </div>
          <div>
            <h2 className='text-base sm:text-xl font-bold text-gray-900 dark:text-white flex items-center gap-1.5 sm:gap-2'>
              {t('aiIntelligence.title')}
              <span className='px-1.5 sm:px-2 py-0.5 text-[8px] sm:text-[10px] font-bold bg-gradient-to-r from-blue-500 to-purple-600 text-white rounded-full uppercase'>
                Pro
              </span>
            </h2>
        ...
        <div
          className={`px-2 sm:px-4 py-1 sm:py-2 rounded-lg sm:rounded-xl text-xs sm:text-sm font-bold shadow-sm ${
            portfolioScore >= 80
        ...
        >
          <div className='flex items-center gap-1 sm:gap-2'>
            ...
            <span className='hidden sm:inline'>Score:</span> {portfolioScore}%
          </div>
        </div>
      </div>
          ...
          value={
            swapData?.summary.health_status === 'HEALTHY'
              ...
              : swapData?.summary.health_status === 'MODERATE'
                ...
                : swapData?.summary.health_status === 'NEEDS_ATTENTION'
                  ? t('aiIntelligence.swap.health.needsAttention')
                  : t('aiIntelligence.swap.health.poor')
          }
          color={
            swapData?.summary.health_status === 'HEALTHY'
              ? 'text-green-500'
      ...
      {insights.length > 0 && (
        <div className='relative space-y-2 sm:space-y-3'>
          <h3 className='text-xs sm:text-sm font-semibold text-gray-700 dark:text-gray-300 flex items-center gap-1.5 sm:gap-2 mb-2 sm:mb-3'>
      ...
      {!athData && !correlationData && !swapData && (
        <div className='relative flex flex-col items-center justify-center py-6 sm:py-8 text-gray-500 dark:text-gray-400'>
          <div className='p-3 sm:p-4 bg-gray-100 dark:bg-gray-800 rounded-full mb-3 sm:mb-4'>
            <Brain className='w-8 h-8 sm:w-10 sm:h-10 opacity-50' />
          </div>
      ...
      {/* Powered by AI badge */}
      <div className='relative mt-4 sm:mt-6 pt-3 sm:pt-4 border-t border-gray-200 dark:border-gray-700/50 flex items-center justify-center gap-1.5 sm:gap-2 text-[10px] sm:text-xs text-gray-400 dark:text-gray-500'>
