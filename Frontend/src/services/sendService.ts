...
class SendService {
  ...
  async validateAddress(data: ValidateAddressRequest): Promise<ValidateAddressResponse> {
    try {
      ...
      const response = await apiClient.post<ValidateAddressResponse>('/wallets/validate-address', data);
   ...
   */
  async sendCustodial(data: Omit<SendTransactionRequest, 'mode'>): Promise<SendTransactionResponse> {
   ...
   */
  async sendNonCustodial(data: Omit<SendTransactionRequest, 'mode'>): Promise<SendTransactionResponse> {
  ...
  async getTransactionStatus(transactionId: number): Promise<TransactionStatusResponse> {
    const response = await apiClient.get<TransactionStatusResponse>(`/transactions/${transactionId}/status`);
    return response.data;
  }
}
