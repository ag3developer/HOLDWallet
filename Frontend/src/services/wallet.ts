...
class WalletService {
  ...
  // Update wallet name
  async updateWallet(walletId: string, updates: Partial<Pick<Wallet, 'name' | 'isActive'>>): Promise<Wallet> {
  ...
  // Get wallet balance
  async getWalletBalance(walletId: string, refresh?: boolean): Promise<{ balance: string; balanceUSD: string }> {
    const params = refresh ? { refresh: 'true' } : {}
    const response = await apiClient.get<ApiResponse<{ balance: string; balanceUSD: string }>>(`/wallets/${walletId}/balance`, { params })
    return response.data.data
  }
  ...
  async getReceiveAddress(walletId: string): Promise<{ address: string; qrCode: string }> {
    const response = await apiClient.get<ApiResponse<{ address: string; qrCode: string }>>(`/wallets/${walletId}/receive`)
    return response.data.data
  }

  // Send transaction
  async sendTransaction(transactionData: SendTransactionRequest): Promise<Transaction> {
    const response = await apiClient.post<ApiResponse<Transaction>>('/transactions/send', transactionData)
    return response.data.data
  }

  // Estimate transaction fee
  async estimateFee(walletId: string, toAddress: string, amount: string, priority: 'low' | 'medium' | 'high' = 'medium'): Promise<{ fee: string; feeUSD: string }> {
    const response = await apiClient.post<ApiResponse<{ fee: string; feeUSD: string }>>('/transactions/estimate-fee', {
      walletId,
      toAddress,
      amount,
      priority
  ...
  async getWalletTransactions(
    ...
    const response = await apiClient.get<PaginatedResponse<Transaction>>(`/wallets/${walletId}/transactions`, { params })
  ...
  async cancelTransaction(transactionId: string): Promise<Transaction> {
    const response = await apiClient.post<ApiResponse<Transaction>>(`/transactions/${transactionId}/cancel`)
    return response.data.data
  }

  // Resend failed transaction
  async resendTransaction(transactionId: string): Promise<Transaction> {
    const response = await apiClient.post<ApiResponse<Transaction>>(`/transactions/${transactionId}/resend`)
    return response.data.data
  }

  // Get supported coins
  async getSupportedCoins(): Promise<{ coin: string; name: string; icon: string; decimals: number; networks: string[] }[]> {
    const response = await apiClient.get<ApiResponse<{ coin: string; name: string; icon: string; decimals: number; networks: string[] }[]>>('/wallets/supported-coins')
    return response.data.data
  }

  // Validate wallet address
  async validateAddress(coin: string, address: string): Promise<{ isValid: boolean; network?: string }> {
    const response = await apiClient.post<ApiResponse<{ isValid: boolean; network?: string }>>('/wallets/validate-address', {
      coin,
      address
    })
    return response.data.data
  ...
  async exportWallet(walletId: string, password: string): Promise<{ privateKey?: string; mnemonic?: string }> {
    const response = await apiClient.post<ApiResponse<{ privateKey?: string; mnemonic?: string }>>(`/wallets/${walletId}/export`, {
      password
    })
    return response.data.data
  }
  ...
  async backupWallet(walletId: string): Promise<{ backupData: string; checksum: string }> {
    const response = await apiClient.get<ApiResponse<{ backupData: string; checksum: string }>>(`/wallets/${walletId}/backup`)
    return response.data.data
  }
  ...
  async getWalletStats(walletId: string, period: '24h' | '7d' | '30d' | '1y' = '30d'): Promise<{
