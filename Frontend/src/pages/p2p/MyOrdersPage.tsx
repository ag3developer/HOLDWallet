...
export const MyOrdersPage = () => {
  ...
  const [activeTab, setActiveTab] = useState<'active' | 'paused' | 'completed' | 'cancelled'>('active')
  ...
  const getStatusBadge = (status: string) => {
    const badges = {
      active: { icon: CheckCircle, text: 'Ativa', color: 'bg-green-100 text-green-700 dark:bg-green-900 dark:text-green-300' },
      paused: { icon: Pause, text: 'Pausada', color: 'bg-yellow-100 text-yellow-700 dark:bg-yellow-900 dark:text-yellow-300' },
      completed: { icon: CheckCircle, text: 'Completa', color: 'bg-blue-100 text-blue-700 dark:bg-blue-900 dark:text-blue-300' },
      cancelled: { icon: XCircle, text: 'Cancelada', color: 'bg-red-100 text-red-700 dark:bg-red-900 dark:text-red-300' },
    }
  ...
  const filteredOrders = ordersData?.data?.filter((order: any) => {
    ...
    const matchesSearch = !searchTerm || 
      ...
      order.payment_methods?.some((pm: string) => pm.toLowerCase().includes(searchTerm.toLowerCase()))
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                        ...
                        <div>
                          ...
                          <p className="font-semibold text-gray-900 dark:text-white text-sm">
                            {formatCurrency(order.min_amount)} - {formatCurrency(order.max_amount)}
                          </p>
                        </div>
                        <div>
                          <p className="text-xs text-gray-600 dark:text-gray-400 mb-1">Trades</p>
                          <p className="font-semibold text-gray-900 dark:text-white">
                            {order.trades_count || 0} completos
                          </p>
                        </div>
                      </div>
                      ...
                      <div className="flex gap-2">
                        ...
                        {order.status === 'active' && (
                          <>
                            ...
                            <button
                              onClick={() => handleToggleStatus(order.id, order.status === 'active')}
