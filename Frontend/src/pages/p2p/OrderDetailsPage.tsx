...
export const OrderDetailsPage = () => {
  ...
  const handleStartTrade = async () => {
    ...
    if (amount < minAmount || amount > maxAmount) {
      toast.error(`Valor deve estar entre ${formatCurrency(minAmount)} e ${formatCurrency(maxAmount)}`)
      return
    }
    ...
    try {
      ...
      navigate(`/p2p/trade/${result.data.id}`)
    } catch (error) {
          ...
          <div className="space-y-4">
            ...
            <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-lg border border-gray-200 dark:border-gray-700">
              <div className="p-4 sm:p-6">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
                  {/* Preço */}
                  <div>
                    <div className="text-xs text-gray-500 dark:text-gray-400 mb-1 font-medium">Preço Unitário</div>
                    <div className="text-2xl sm:text-3xl font-bold text-gray-900 dark:text-white">
                      {formatCurrency(parseFloat(order.price))}
                    </div>
                    <div className="text-xs text-gray-500 dark:text-gray-400 mt-1">por {order.coin}</div>
                  </div>

                  {/* Quantidade */}
                  <div>
                    <div className="text-xs text-gray-500 dark:text-gray-400 mb-1 font-medium">Quantidade Disponível</div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                {order.paymentMethods && order.paymentMethods.length > 0 ? (
                  order.paymentMethods.map((method: any) => {
                    ...
                    return (
                      <div key={method.id} className="flex items-center gap-2 p-3 rounded-xl bg-gradient-to-br from-blue-50 to-blue-100 dark:from-blue-900/20 dark:to-blue-800/20 border border-blue-200 dark:border-blue-700">
                        <div className="w-8 h-8 rounded-lg bg-blue-500 flex items-center justify-center flex-shrink-0">
                          ...
                          {(typeUpper === 'MERCADO_PAGO' || typeUpper === 'PAYPAL') && <Wallet className="w-4 h-4 text-white" />}
                        </div>
                        <div className="flex-1 min-w-0">
                          <div className="text-xs font-bold text-blue-900 dark:text-blue-100 uppercase">{typeUpper}</div>
                          <div className="text-xs text-blue-700 dark:text-blue-300 truncate">
                            {details?.keyValue || details?.account || details?.email || details?.identifier || 'N/A'}
                          </div>
                        </div>
                      </div>
                    )
                  ...
                  <div className="col-span-2 text-center py-4 text-sm text-gray-500">Nenhum método configurado</div>
                )}
              </div>
            </div>
            ...
            <div className="hidden sm:block bg-white dark:bg-gray-800 rounded-2xl shadow-lg border border-gray-200 dark:border-gray-700 p-5">
              <div className="flex items-center justify-between mb-3">
                ...
                <span className="text-xs text-gray-500 dark:text-gray-400">{order.completedTrades || 0} trades</span>
              </div>
              <div className="text-center py-6">
                <Activity className="w-10 h-10 text-gray-300 dark:text-gray-600 mx-auto mb-2" />
                <p className="text-xs text-gray-500 dark:text-gray-400">Ainda não há trades para esta ordem</p>
              </div>
            </div>
          </div>
            ...
            <div className="bg-gradient-to-br from-white to-gray-50 dark:from-gray-800 dark:to-gray-900 rounded-2xl shadow-xl border border-gray-200 dark:border-gray-700 p-4 sm:p-6">
              ...
              <div className="grid grid-cols-2 gap-2 sm:gap-3 mb-4 sm:mb-6">
                <div className="text-center p-2 sm:p-3 rounded-xl bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800">
                  ...
                  <div className="text-base sm:text-lg font-bold text-gray-900 dark:text-white">{order.avgRating || 0}%</div>
                ...
                <div className="text-center p-2 sm:p-3 rounded-xl bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800">
                  ...
                  <div className="text-base sm:text-lg font-bold text-gray-900 dark:text-white">{order.completedTrades || 0}</div>
                ...
                <div className="text-center p-2 sm:p-3 rounded-xl bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800">
                  ...
                  <div className="text-base sm:text-lg font-bold text-gray-900 dark:text-white">{order.successRate || 0}%</div>
                ...
                <div className="text-center p-2 sm:p-3 rounded-xl bg-purple-50 dark:bg-purple-900/20 border border-purple-200 dark:border-purple-800">
                  ...
                  <div className="text-base sm:text-lg font-bold text-gray-900 dark:text-white">2024</div>
              ...
              {order.badges && order.badges.length > 0 && (
                <div className="mb-4 sm:mb-6">
                  <div className="text-xs text-gray-500 dark:text-gray-400 mb-2 font-medium">Badges</div>
                  <div className="flex flex-wrap gap-2">
                    {order.badges.map((badge: string) => (
                      ...
                      >
                        {badge === 'verified' && <><Shield className="w-3 h-3" /> Verificado</>}
                        {badge === 'pro_trader' && <><Zap className="w-3 h-3" /> Pro</>}
                        {badge === 'trusted' && <><Award className="w-3 h-3" /> Confiável</>}
                      </span>
                    ))}
                  </div>
                </div>
              ...
              <div className="space-y-3 sm:space-y-4">
                ...
                <div>
                  ...
                  <div className="text-xs mt-1.5 opacity-75">
                    Limites: {formatCurrency(parseFloat(order.minAmount))} - {formatCurrency(parseFloat(order.maxAmount))}
                  </div>
                </div>
                ...
                {tradeAmount && parseFloat(tradeAmount) > 0 && (
                  <div className="space-y-2 p-3 sm:p-4 rounded-xl bg-white/10 backdrop-blur-sm border border-white/20">
                    <div className="flex justify-between text-xs sm:text-sm">
                      ...
                      <span className="font-bold">{(parseFloat(tradeAmount) / parseFloat(order.price)).toFixed(8)} {order.coin}</span>
                    </div>
                    <div className="flex justify-between text-xs sm:text-sm">
                ...
                <button
                  ...
                  disabled={!tradeAmount || parseFloat(tradeAmount) < parseFloat(order.minAmount) || parseFloat(tradeAmount) > parseFloat(order.maxAmount) || isStartingTrade}
