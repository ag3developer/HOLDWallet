...
export const P2PPage = () => {
  ...
  const { data: ordersData, isLoading: ordersLoading, error: ordersError, refetch: refetchOrders } = useP2POrders({
    type: activeTab,
    coin: selectedCrypto,
  ...
  const { data: marketStats } = useMarketStats(selectedCrypto)
  ...
  const getPaymentMethodIcon = (method: string) => {
    switch(method) {
      ...
      case 'Mercado Pago': return <CreditCard className="w-4 h-4" />
                      ...
                      <div className="flex items-center space-x-3">
                        ...
                        <div>
                          ...
                          <div className="flex items-center gap-2 mt-1">
                            ...
                            <span className="text-xs text-gray-600 dark:text-gray-400">
                              {order.user?.completed_trades || 0} trades
                            </span>
                          </div>
                          {order.user?.badges && order.user.badges.length > 0 && (
                            <div className="flex items-center gap-1 mt-1">
                              {order.user.badges.slice(0, 2).map((badge: string, index: number) => (
                                <div key={index} className="inline-flex items-center gap-1 px-2 py-1 bg-blue-100 dark:bg-blue-900 text-blue-700 dark:text-blue-300 rounded-full text-xs">
                                  {getBadgeIcon(badge)}
                    ...
                    <td className="px-6 py-4">
                      <div className="flex flex-wrap gap-1">
                        {order.payment_methods?.map((method: string, index: number) => (
                          <div key={index} className="inline-flex items-center gap-1 px-2 py-1 bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300 rounded text-xs">
