...
type Step = 'choose' | 'create-form' | 'restore-form' | 'mnemonic-display' | 'mnemonic-confirm' | 'success'
...
`
  ...
  const renderStepIndicator = () => {
    const steps = walletType === 'create' 
      ? ['Escolher Tipo', 'Formulário', 'Frase de Recuperação', 'Confirmar Frase', 'Concluído']
      : ['Escolher Tipo', 'Restaurar', 'Concluído']
    
    const currentStepIndex = walletType === 'create'
      ? ['choose', 'create-form', 'mnemonic-display', 'mnemonic-confirm', 'success'].indexOf(currentStep)
          ...
          {steps.map((step, index) => (
            <React.Fragment key={index}>
              ...
              {index < steps.length - 1 && (
                <div className={`w-8 sm:w-12 h-0.5 sm:h-1 mx-1 sm:mx-2 flex-shrink-0 ${
        ...
        <div className="flex items-center gap-2 sm:gap-4 mb-6 sm:mb-8">
          ...
          <div className="flex-1 min-w-0">
            <h1 className="text-2xl sm:text-3xl font-bold text-gray-900 dark:text-white truncate">
              {walletType === 'create' ? 'Criar Carteira Principal' : 'Restaurar Carteira Principal'}
            </h1>
              ...
              <div className="grid grid-cols-1 gap-4 sm:gap-6 max-w-2xl mx-auto">
                ...
                >
                  ...
                  <p className="text-sm text-gray-600 dark:text-gray-400 px-2">
                    Crie uma carteira HD que suporta todas as criptomoedas com uma única frase de recuperação
                  </p>
                </button>
              ...
              <div className="space-y-3 sm:space-y-4">
                ...
                <div className="space-y-2">
                  <div className="flex items-center justify-between">
                    ...
                    >
                      {showMnemonic ? <EyeOffIcon className="w-3 h-3 sm:w-4 sm:h-4" /> : <EyeIcon className="w-3 h-3 sm:w-4 sm:h-4" />}
                      <span className="hidden sm:inline">{showMnemonic ? 'Ocultar' : 'Mostrar'}</span>
                    </button>
                  </div>
                  ...
                  <div className="relative">
                    <div className="p-3 sm:p-6 bg-gray-50 dark:bg-gray-700 rounded-lg border-2 border-dashed border-gray-300 dark:border-gray-600 min-h-[120px] sm:min-h-[160px]">
                      {showMnemonic ? (
                        <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 sm:gap-4">
                          {createdWallet.mnemonic?.split(' ').map((word, index) => (
                            <div key={index} className="flex items-center space-x-2 p-2 bg-white dark:bg-gray-800 rounded border">
                              <span className="text-gray-500 text-xs font-mono w-4 sm:w-6">{index + 1}.</span>
                        ...
                        <div className="text-center text-gray-500 py-8 sm:py-16">
                          ...
                          <p className="text-sm sm:text-base">Clique em "Mostrar" para revelar a frase de recuperação</p>
                        </div>
                      )}
                    </div>
                    ...
                    {showMnemonic && (
                      ...
                      >
                        ...
                        <span className="hidden sm:inline">{copiedMnemonic ? 'Copiado!' : 'Copiar'}</span>
                      </button>
                    )}
                  </div>
                </div>
                ...
                {randomWordIndices.map((wordIndex) => (
                  <div key={wordIndex} className="space-y-2">
                    ...
                    <input
                      ...
                      onChange={(e) => setConfirmationWords(prev => ({
                        ...prev,
